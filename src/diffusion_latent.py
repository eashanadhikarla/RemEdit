...
class Asyrp(object):
    ...
    def run_training(self):
        ...
        if self.args.train_delta_block:
            ...
            ###############
            # printmodel = model.to(self.device)
            ###############
        ...
        # Temporary testing of model architecture
        # print(f"\n\n{printmodel}")
        # exit(0)
        ...
        for it_out in range(self.args.start_iter_when_you_use_pretrained, self.args.n_iter):
            ...
            if self.args.do_train:
                ...
                if self.args.retrain==0 and os.path.exists(save_name):
                    ...
                    # Unfortunately, ima_lat_pairs_dic does not match with batch_s1``ize
                    ...
                    for step, (x0, _, x_lat) in enumerate(img_lat_pairs_dic['train']):
                        ...
                        if x_lat_tensor is None:
                            ...
                            # print(f"step: {step}, x_lat_tensor.shape: {x_lat_tensor.shape}, x_lat.shape: {x_lat.shape}")
                            x_lat_tensor = torch.cat((x_lat_tensor.to(self.device), x_lat.to(self.device)), dim=0)
                            if self.args.use_x0_tensor:
                                x0_tensor = torch.cat((x0_tensor.to(self.device), x0.to(self.device)), dim=0)
    ...
    def compute_lpips_distance(self):
        ...
        train_dataset, test_dataset = get_dataset(self.config.data.dataset, 
                                                  DATASET_PATHS, 
                                                  self.config,
                                                  target_class_num = self.args.target_class_num
                                                )

        loader_dic = get_dataloader(train_dataset, 
                                    test_dataset, 
                                    bs_train = 1,
            ...
            with torch.no_grad():
                with tqdm(total=len(seq_inv), desc=f"Inversion process {step}") as progress_bar:
                    for it, (i, j) in enumerate(zip((seq_inv_next[1:]), (seq_inv[1:]))):
                        ...
                        if save_imgs:
                            tvu.save_image((x + 1) * 0.5, os.path.join(self.args.image_folder, f'LPIPS_{step}_{j}.png'))
                            tvu.save_image((x0_t + 1) * 0.5, os.path.join(self.args.image_folder, f'X0_t_LPIPS_{step}_{j}.png'))
                        progress_bar.update(1)
