...
class DeltaBlock(nn.Module):
    def __init__(
        ...
        if use_midblock:
            ...
            # self.in_layer = get_dh_layer(self.layer_type, nheads, num_layers, dim_feedforward, dropout)
            # self.in_layer = nn.Conv2d(512, 512, kernel_size=1, stride=1, padding=0)
            self.in_layer = Mamba(d_model=512, d_state=32, d_conv=4, expand=4)

            self.temb_proj = nn.Linear(temb_channels, out_channels)
            self.norm2 = Normalize(out_channels)

            # self.out_layer = get_dh_layer(self.layer_type, nheads, num_layers, dim_feedforward, dropout)
            # self.out_layer = nn.Conv2d(512, 512, kernel_size=1, stride=1, padding=0)
            self.out_layer = Mamba(d_model=512, d_state=32, d_conv=4, expand=4)
            self.out_layer_final = nn.Conv2d(512, 512, kernel_size=1, stride=1, padding=0)
    ...
    def forward(self, x, temb=None):
        ...
        ### This location for riemannian block didn't lead in edits that we intended to do. 
        # # Enhanced Riemannian manifold block to compute delta_h
...
# # %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# # Mamba1000_lr0.05 (best results so far)
# # %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
